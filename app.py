...
class User(UserMixin, db.Model):
    ...
    # Новые поля:
    birth_date = db.Column(db.String(20))   # можно заменить на db.Date, если нужен тип "дата"
...
class Log(db.Model):
    ...
    ip_address = db.Column(db.String(45))  # для IPv4/IPv6
...
class AdminCreationForm(FlaskForm):
    ...
    birth_date = StringField('Дата рождения', validators=[DataRequired()])  # Можно добавить валидатор даты
...
def add_log(action):
    if current_user.is_authenticated:
        ip_address = request.remote_addr or "неизвестно"
        user_agent = request.headers.get('User-Agent') or "неизвестно"
        log_entry = Log(
...
def all_records():
    # Получаем строку поиска из параметра 'name'
    name_query = request.args.get('name', '').strip()

    # Если строка поиска не пустая, фильтруем записи по full_name (без учёта регистра)
    if name_query:
        records = Record.query.filter(Record.full_name.ilike(f"%{name_query}%")).all()
    else:
        records = Record.query.all()

    return render_template('all_records.html', records=records, name_query=name_query)
...
def create_record():
    if request.method == 'POST':
        ...
        if photo_file and photo_file.filename:
            ...
            # Сохраняем в базе только относительный путь
            photo_path = f"images/{filename}"
        else:
            photo_path = None

        # Создаём объект записи и сохраняем его в базе
        record = Record(
            full_name=full_name,
            # другие поля...
...
def create_admin():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Новый администратор успешно создан!", "success")
        add_log(f"Создан новый администратор: {new_admin.email}")
        return redirect(url_for('manage_users'))
    return render_template('admin_create_admin.html', form=form)

@app.route('/cards/gallery')
def cards_gallery():
    # Получаем строку поиска (по боевым действиям)
    conflict_query = request.args.get('conflict', '').strip()
    # Получаем номер страницы (по умолчанию 1)
    page = request.args.get('page', 1, type=int)

    # Базовый запрос: выбираем записи только со статусом "approved"
    query = Record.query.filter_by(status='approved')

    # Если пользователь ввёл запрос, фильтруем по military_service (без учёта регистра)
    if conflict_query:
        query = query.filter(Record.military_service.ilike(f"%{conflict_query}%"))

    # Разбиваем на страницы (по 20 карточек на страницу)
    pagination = query.paginate(page=page, per_page=20, error_out=False)
    cards = pagination.items  # список записей для текущей страницы

    return render_template(
        'cards_gallery.html',
        cards=cards,               # список карточек
        pagination=pagination,     # объект пагинации для построения ссылок
        conflict_query=conflict_query  # чтобы форма не теряла введённое значение
    )
